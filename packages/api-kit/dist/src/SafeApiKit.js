...
class SafeApiKit {
  constructor({ txServiceUrl, ethAdapter }) {
    ...
    __classPrivateFieldSet(
      ...
      (0, utils_1.getTxServiceBaseUrl)(txServiceUrl),
      'f'
    )
  ...
  async getSafesByOwner(ownerAddress) {
    ...
    const { address } = await __classPrivateFieldGet(
      this,
      _SafeApiKit_ethAdapter,
      'f'
    ).getEip3770Address(ownerAddress)
    return (0, httpRequests_1.sendRequest)({
      url: `${__classPrivateFieldGet(
        this,
        _SafeApiKit_txServiceBaseUrl,
  ...
  async getSafesByModule(moduleAddress) {
    ...
    const { address } = await __classPrivateFieldGet(
      ...
      _SafeApiKit_ethAdapter,
      'f'
    ).getEip3770Address(moduleAddress)
    return (0, httpRequests_1.sendRequest)({
      url: `${__classPrivateFieldGet(
        this,
        _SafeApiKit_txServiceBaseUrl,
  ...
  async getSafeDelegates({ safeAddress, delegateAddress, delegatorAddress, label, limit, offset }) {
    ...
    if (safeAddress) {
      const { address: safe } = await __classPrivateFieldGet(
        this,
        _SafeApiKit_ethAdapter,
        'f'
      ).getEip3770Address(safeAddress)
    ...
    if (delegateAddress) {
      const { address: delegate } = await __classPrivateFieldGet(
        this,
        _SafeApiKit_ethAdapter,
        'f'
      ).getEip3770Address(delegateAddress)
      url.searchParams.set('delegate', delegate)
    }
    if (delegatorAddress) {
      const { address: delegator } = await __classPrivateFieldGet(
        this,
        _SafeApiKit_ethAdapter,
        'f'
      ).getEip3770Address(delegatorAddress)
  ...
  async addSafeDelegate({ safeAddress, delegateAddress, delegatorAddress, label, signer }) {
    ...
    const body = {
      safe: safeAddress
        ? (
            await __classPrivateFieldGet(this, _SafeApiKit_ethAdapter, 'f').getEip3770Address(
              safeAddress
            )
          ).address
        : null,
  ...
  async proposeTransaction({
    ...
    const { address: sender } = await __classPrivateFieldGet(
      this,
      _SafeApiKit_ethAdapter,
      'f'
    ).getEip3770Address(senderAddress)
    if (safeTxHash === '') {
      throw new Error('Invalid safeTxHash')
    }
    return (0, httpRequests_1.sendRequest)({
  ...
  async getAllTransactions(safeAddress, options) {
    ...
    const trusted =
      ((_a = options === null || options === void 0 ? void 0 : options.trusted) === null ||
      _a === void 0
        ? void 0
        : _a.toString()) || 'true'
    url.searchParams.set('trusted', trusted)
    const queued =
      ((_b = options === null || options === void 0 ? void 0 : options.queued) === null ||
      _b === void 0
        ? void 0
        : _b.toString()) || 'true'
    url.searchParams.set('queued', queued)
    const executed =
      ((_c = options === null || options === void 0 ? void 0 : options.executed) === null ||
      _c === void 0
        ? void 0
        : _c.toString()) || 'false'
  ...
  async getToken(tokenAddress) {
    ...
    const { address } = await __classPrivateFieldGet(
      ...
      _SafeApiKit_ethAdapter,
      'f'
    ).getEip3770Address(tokenAddress)
    return (0, httpRequests_1.sendRequest)({
      url: `${__classPrivateFieldGet(
        this,
        _SafeApiKit_txServiceBaseUrl,
