...
class OwnerManager {
  ...
  validateOwnerAddress(ownerAddress, errorMessage) {
    const isValidAddress = __classPrivateFieldGet(this, _OwnerManager_ethAdapter, 'f').isAddress(
      ownerAddress
    )
  ...
  async encodeAddOwnerWithThresholdData(ownerAddress, threshold) {
    ...
    const newThreshold =
      threshold !== null && threshold !== void 0 ? threshold : await this.getThreshold()
    this.validateThreshold(newThreshold, owners.length + 1)
    return __classPrivateFieldGet(this, _OwnerManager_safeContract, 'f').encode(
  ...
  async encodeRemoveOwnerData(ownerAddress, threshold) {
    ...
    const newThreshold =
      threshold !== null && threshold !== void 0 ? threshold : (await this.getThreshold()) - 1
    this.validateThreshold(newThreshold, owners.length - 1)
    const prevOwnerAddress =
      ownerIndex === 0 ? constants_1.SENTINEL_ADDRESS : owners[ownerIndex - 1]
  ...
  async encodeSwapOwnerData(oldOwnerAddress, newOwnerAddress) {
    ...
    const prevOwnerAddress =
      oldOwnerIndex === 0 ? constants_1.SENTINEL_ADDRESS : owners[oldOwnerIndex - 1]
