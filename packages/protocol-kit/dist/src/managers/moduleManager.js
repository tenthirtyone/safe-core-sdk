...
class ModuleManager {
  ...
  validateModuleAddress(moduleAddress) {
    const isValidAddress = __classPrivateFieldGet(this, _ModuleManager_ethAdapter, 'f').isAddress(
      moduleAddress
    )
  ...
  validateModuleIsNotEnabled(moduleAddress, modules) {
    const moduleIndex = modules.findIndex((module) =>
      (0, address_1.sameString)(module, moduleAddress)
    )
  ...
  async isModuleEnabled(moduleAddress) {
    ...
    return __classPrivateFieldGet(this, _ModuleManager_safeContract, 'f').isModuleEnabled(
      moduleAddress
    )
  }
  ...
  async encodeDisableModuleData(moduleAddress) {
    ...
    const prevModuleAddress =
      moduleIndex === 0 ? constants_1.SENTINEL_ADDRESS : modules[moduleIndex - 1]
