...
class GuardManager {
  ...
  validateGuardAddress(guardAddress) {
    const isValidAddress = __classPrivateFieldGet(this, _GuardManager_ethAdapter, 'f').isAddress(
      guardAddress
    )
  ...
  async getGuard() {
    ...
    const safeVersion = await __classPrivateFieldGet(
      this,
      _GuardManager_safeContract,
      'f'
    ).getVersion()
    if ((0, utils_1.hasSafeFeature)(utils_1.SAFE_FEATURES.SAFE_TX_GUARDS, safeVersion)) {
      return __classPrivateFieldGet(this, _GuardManager_ethAdapter, 'f').getStorageAt(
        __classPrivateFieldGet(this, _GuardManager_safeContract, 'f').getAddress(),
        __classPrivateFieldGet(this, _GuardManager_slot, 'f')
      )
    } else {
      throw new Error(
...
;(_GuardManager_ethAdapter = new WeakMap()),
  (_GuardManager_safeContract = new WeakMap()),
  (_GuardManager_slot = new WeakMap())
